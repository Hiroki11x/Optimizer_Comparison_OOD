...
if __name__ == "__main__":
    ...
    #update for weight decay search
    wandb_project_name = "{}_{}_{}_with_wd".format(hparams['algorithm'], 
                                             hparams['dataset'],
                                             hparams['optimizer_name'])
    if (args.test_envs)[0] != 0:
        wandb_project_name = "{}_{}".format(wandb_project_name, (args.test_envs)[0])
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            for name, loader, weights in evals:
                ...
                if ('out'in name) and not (str((args.test_envs)[0]) in name):
                    avg_val_acc.append(acc)
                if (str((args.test_envs)[0]) in name):
                    if ('out'in name) :
                        avg_test_acc += acc * args.holdout_fraction
                    elif ('in'in name) :
                        avg_test_acc += acc * (1 - args.holdout_fraction)
            results['avg_val_acc'] = sum(avg_val_acc)/len(avg_val_acc)
            results['avg_test_acc'] = avg_test_acc
            ...
            for key in results_keys:
                # print('k:{} /v: {}'.format(key, results[key]))
                wandb.log(
